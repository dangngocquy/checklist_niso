...
const XuLyPhanHoi = ({ t, user, hasxemPermission, hasxlphPermission }) => {
    ...
    () =>
      debounce(async (page, size, tab, currentFilters) => {
        ...
        try {
          ...
          const response = await axios.get('/traloi/response-all', {
            params: apiFilters,
            headers: {
          ...
          const processedTraloiData = traloiData.map((item) => ({
            ...
            yeucauxuly: item.yeucauxuly || false, // Đảm bảo trường yeucauxuly được xử lý
          }));
  ...
  useEffect(() => {
    if (!isMounted.current) {
      ...
      if (!validTabs.includes(tab)) {
        navigate(`/auth/docs/list/view/xu-ly-phan-hoi/assigned`, { replace: true });
      }
  ...
  const handleTabChange = useCallback(
    (key) => {
      ...
      navigate(`/auth/docs/list/view/xu-ly-phan-hoi/${key}`, { replace: true });
    },
  ...
  const showViewDrawer = useCallback(
    async (record) => {
      ...
      // Only update watching if user.keys is not already in the array
      ...
      },
      {
        ...
        render: (text, record) => {
          ...
          const userForCard = {
            ...
            username: matchedUser?.username || record.username || 'N/A',
...
export default React.memo(XuLyPhanHoi);