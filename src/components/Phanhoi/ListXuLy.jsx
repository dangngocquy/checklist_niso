...
import { List, message, Avatar, Tooltip, Skeleton, Empty, Button, Typography, Tabs, Select, DatePicker, Space, Alert } from "antd";
...
const ListXuLy = React.memo(({ user, t }) => {
        ...
        return (
            <Tooltip title={matchedUser?.name || "Unknown User"}>
                {matchedUser?.imgAvatar ? (
                    ...
                    <Avatars user={{ name: matchedUser.name }} sizeImg="large" />
                )}
            </Tooltip>
        );
    };
    ...
    const filteredData = useMemo(() => {
        return data.filter(item => {
            ...
            return matchesTitle && matchesCauhoi && matchesDate && matchesTab;
        });
    }, [data, selectedTitle, selectedCauhoi, startDate, endDate, activeTab]);
